...
> **本次发布最终形态**：白色主界面 + 中性墨色（无主题色）+ **淡彩图标（纯色，全项目无渐变）** + 界面无毛玻璃（仅底部/侧边导航栏保留液态玻璃）+ 启动欢迎首屏 4 页轮播（向上拉取出场）。以下按迭代顺序记录，最终形态以最后几条为准。

- **去主题色，改白底 + 中性墨色**：全项目不再使用可自定义的主题色。`accentColorNotifier`（约 60 个文件读取，保留名称与 API 以免大范围改动）语义变更为**中性墨色**——浅色模式近黑 `kInkLight(#111114)`、深色模式近白 `kInkDark(#F5F4F2)`，由 `main.dart` 的 `_syncInkColor()` 按当前明暗模式自动同步（`initState` 首帧后 / `setThemeMode` / `didChangePlatformBrightness`）。随之调整：`ColorScheme.primary/onPrimary`（深色墨色时 `onPrimary` 用近黑）、`ElevatedButtonTheme`（墨色实心按钮：浅色黑底白字 / 深色白底黑字）、`BottomNavigationBarTheme.selectedItemColor`、输入框聚焦边框；背景渐变改为「纯白 → 极浅冷灰」（`core/liquid_background.dart`），气泡改为固定低饱和多彩色（与主题色解耦）。**设置页**：移除「主题颜色」（12 色板 + HSV 自定义调色盘 + `accent_color` 持久化读写全部删除，`colorToHex`/`hexToColor` 死代码一并清理），外观页仅保留明暗模式与自定义背景；设置页「外观」副标题同步改为「明暗模式 · 自定义背景」。
- **彩色图标体系**：新增模块专属色表 `kModuleColors`（`lib/home/app_data.dart`，39 个模块一模块一色相，`AppEntry.color` 按名称取色、未登记回退中性灰），色相只用于图标自身、不再作为界面主色。`core/ios_kit.dart` 的 `appTileGlass` 新增 `fill` 参数：传入模块色时渲染**彩色实心方块**（同色系 18% 加深对角渐变 + 白色图形 + 同色柔光投影），不传则保持原玻璃观感（向后兼容）。应用宫格（`lib/home/main_screen.dart` `_buildAppCard`）、首页「常用功能」宫格（`_QuickAppTile`）、首页卡片头部图标（`home_dashboard._cardHeaderIcon` 新增 `color` 参数）、设置页列表（外观/常用功能/首页卡片/倒计时/摸鱼日历/登录账号/检查更新等行）、常用功能选择弹窗、资讯附件与网络服务下载图标全部改为彩色；游客锁定的宫格条目降级为中性灰。
- **取消界面毛玻璃，只保留液态导航栏**：新增统一材质配置 `lib/core/glass_style.dart`。原理依据库的设置解析优先级 `effectiveSettings = widget.settings ?? GlassTheme.settingsFor(context)`——把**全局 `GlassTheme` 置为实色**（`kAppGlassTheme`：不透明白/近黑 `glassColor` + `thickness/blur/chromaticAberration/lightIntensity/ambientStrength/fresnelStrength` 全部归零 + `refractiveIndex 1.0`），所有未显式传参的库存玻璃组件（筛选按钮、操作按钮、列表条目、卡片、分段控件、底部弹窗）自动变实色；**导航栏自带显式液态参数**（`GlassTabBar.bottom` 的 `thickness:18/blur:5/refractiveIndex:2.6`、宽屏侧栏手写 `BackdropFilter`）优先级更高，故继续保留液态玻璃。同步实色化自绘面：`contentCardGlass`（卡片/分组列表）、`glassDialog`、加载弹窗、常用功能选择 Sheet、课程详情弹窗、倒计时编辑 Sheet、更新检查弹窗（均去掉 `BackdropFilter` 与半透明渐变/白色高光描边，改为实色 + 极淡描边 + 白底柔和投影，助手函数 `solidSurface/solidHairline/solidShadow`）；`GlassScaffold` 页面层参数改 `kFlatPageSettings`；`main.dart` 的 `cardTheme`/`inputDecorationTheme` 改实色（输入框底色 iOS 分组灰 `#F2F2F7` / 深色 `#2C2C2E`）；课程表卡片去掉 `BackdropFilter` 只留圆角裁剪。
- **新增欢迎首屏（多页轮播）**（`lib/welcome/welcome_page.dart` + `welcome_gate.dart` 新建）：满屏校园实景图 + 顶部压暗渐变保证文字可读 + 两行大标题与副标题 + 底部白色圆角面板内嵌墨色胶囊按钮「开始使用 →」。同日二次迭代：① **改为 4 页横向轮播**（`PageView` + `PageController`），每页独立背景图/标题/副标题——「百川归海 一处相逢」「三江汇流 一城书香」「长江首城 学在宜院」「宜院宾果 与你同行」，随滑动交叉淡入淡出（按页偏移量做透明度 + 位移衰减 + 轻微缩放，非简单随页平移）；② **自动 + 手动滚动**：每 4s 自动切下一页并循环，手指拖拽期间自动播放暂停、松手 3s 后恢复（`NotificationListener<ScrollNotification>` 判定拖拽起止）；③ 面板内新增分页圆点（当前页加宽为墨色胶囊）指示位置，面板与主按钮固定不随页滚动；④ **每次启动都展示**（原先只在首次启动展示一次，`welcome_seen` 标记与 800ms 超时兜底逻辑一并删除，`WelcomeGate` 简化为无状态直出）；⑤ 背景素材换为 4 张校园实景（新增资源 `assets/images/welcome_1..4.jpg`，来源学校官网并已压缩至 1600px 宽 / JPEG q85 控体积，旧 `welcome_campus.png` 删除）；⑥ **修复"图片被压暗"**：原顶部压暗渐变峰值 70% 且覆盖到 62% 屏高、且图片本身按页偏移做透明度交叉淡化（非当前页压到 35%、过渡中当前页也一起变暗）——现遮罩压缩为顶部 34% 内、峰值 30%（仅保证状态栏白色图标可读），**图片恒定全亮**，翻页只淡入淡出文字（文字投影加深补偿可读性）。点击「开始使用」后进入原有 `SplashPage` 会话/登录分流（分流逻辑完全未改动，`main.dart` 的 `home:` 为 `WelcomeGate`）。
- **硬编码品牌色清理**：`course_config.dart` 颜色解析回退值（0xFF191999 → 课程语义蓝 0xFF3B6BFF）、`news_detail_page.dart` 附件图标、`campus_network_service_detail_page.dart` 下载图标改为彩色图标配色；删除死常量 `yibinBlue`。
- **欢迎首屏入场方式改为「向上拉取」（去掉中间过渡屏）**：① **向上拉取**——在欢迎页任意位置向上拖动，整屏（图 + 文字 + 白色面板）随手指 1:1 上移（纯位移，不做透明度衰减）；松手超过屏高 16% 或上滑速度 > 620px/s 即完成入场，否则回弹；点「开始使用」等价于直接完成上滑。② **不再跳转中间页面**：会话/登录分流逻辑由 `main.dart` 的 `SplashPage` 原样迁出为 `lib/splash/startup_flow.dart` 的 `resolveStartupTarget()`（规则一字未改：游客→主界面；有凭据→真实 CAS 重登→无个人信息缓存走信息获取页/否则主界面；无凭据或重登失败→登录页），并改为**在欢迎页展示期间后台静默执行**；入场完成时由 `WelcomeGate` **就地替换根路由内容**为目标页（`setState` 换子组件，无 `Navigator` 跳转、无转场动画）——视觉上就是"拉起欢迎页后应用出现"，彻底不再出现「验证 Cookie 中…」那一屏（用户明确要求「不要跳转到新的登录界面」）。目标未就绪（拉得太快）时先显示极简「正在进入」小转圈，就绪后原地替换。③ 删除已无引用的 `SplashPage`/`_SplashPageState`（约 157 行）及 `main.dart` 中随之下线的 9 个导入；`WelcomeGate` 增加可注入 `resolver`（仅供测试，生产用真实分流）。测试同步重写为两条路径（向上拉取入场 / 点击入场，断言不出现过渡页）——`flutter test` **19/19 通过**。
- **全面取消渐变，改为纯色（"单纯彩色"）**：用户明确「不要使用渐变，使用单纯的彩色」——全项目 `LinearGradient/RadialGradient/SweepGradient` 使用点**清零**（`grep` 校验 0 处）。逐处替换：① 宫格彩色图标 `appTileGlass(fill:)` 由「同色 18% 加深对角渐变 + 同色柔光投影」改为**纯色方块**（仅 `color: 模块色` + 白色图形，无渐变无光晕）；宫格悬停/聚焦反馈由彩色光晕改**中性黑投影**（`main_screen` 与 `ios_kit` 两处）。② 其余自绘面去渐变：`contentCardGlass` 之前已实色化、`glass_filter_chip`（选中=墨色纯色浅底）、`glass_action_button.bgColor`、`main_screen` 侧栏面板/品牌方块/选中胶囊/搜索栏（搜索栏同时改为 iOS 分组灰纯色底）、`safety_page`（2 处）、`xuegong/zhsz_page` 成绩徽章。③ 背景 `LiquidBackground` 由白→浅灰渐变改为**纯白/近黑纯色**，气泡由多彩/霓虹改为**中性浅灰**（保留气泡只为给液态导航栏提供可折射内容）。④ 欢迎首屏**移除顶部压暗渐变遮罩**（图片零压暗）：可读性改由深色状态栏图标（4 张校园图顶部亮度实测 136~237 偏亮）+ 标题/副标题「紧贴投影 + 柔化投影」两层白字描边保证。校验：`dart analyze lib test` 0 error / 0 warning，`flutter test` **19/19 通过**。

- **宫格图标改为「柔和淡彩」风格（更清爽）**：参照用户提供的校园 App 参考图（淡彩底 + 同色系线性图形），`ios_kit.appTileGlass(fill:)` 由「纯色高饱和实心块 + 白色图形」改为**极浅同色淡底（浅色 15% / 深色 22% 不透明度）+ 同色系图形**（深色模式图形提亮 30% 保证对比度），无描边、无投影、无渐变。色表 `kModuleColors` 保持不变（hue 与参考图一致）；设置页列表 / 首页卡片头图 / 附件与下载图标等此前已是同款淡彩（0.12~0.14 淡底 + 原色图形），风格由此统一。`dart analyze lib test` 0 error / 0 warning，`flutter test` 19/19 通过。
...
- **升级 liquid_glass_widgets 0.29.2 → 1.2.3 + jni 1.0.0 → 1.0.3（用户明确要求，解除两项历史锁定）**：`liquid_glass_widgets` 升 0.x→1.x 大版本——`dart analyze` 0 error（glass UI API 用法兼容，无新增问题）、Android `compileDebugKotlin` BUILD SUCCESSFUL；⚠️ 历史 0.29.8 的 FragmentProgram shader 加载曾致 Windows Impeller 白屏，1.2.3 的 Windows 运行时白屏风险**本沙箱无法验证**（仅编译 Android + 静态分析），需真机/Windows 构建实测。`jni` 解除 1.0.0 override 锁定、升 1.0.3——原 1.0.1 的 `kotlin {}` DSL 回归在 1.0.3 已不存在，Android 编译 BUILD SUCCESSFUL（不再报 `Could not find method kotlin()`）；新增传递依赖 `jni_util 1.0.0`。pubspec.yaml 注释同步更新、pubspec.lock 更新。
...
- **新增「校园VPN」功能（服务板块）**：免安装官方 EasyConnect 客户端接入学校深信服 VPN（`https://vpn.yibinu.edu.cn`），协议实现基于开源 zju-connect v1.3.0（参考 HITSZ Connect Verge 方案）。**Android 端**：引入官方 gomobile AAR（`android/app/libs/zju-connect.aar`，`Mobile.login/logout/startStack` API），新建 `YibinVpnService` 前台服务建立 TUN 网卡并把 fd 交给 zju-connect gvisor 栈（分流模式：仅内网网段 10/8、172.16/12、192.168/16 走隧道，MTU 1400，日常上网不受影响）；连接态常驻通知栏。**Windows 端**：首次使用自动下载 zju-connect 内核（直连 + gh-proxy 镜像回退，tar.exe 解压无新依赖），以子进程运行并本地暴露 SOCKS5(127.0.0.1:1080)/HTTP(127.0.0.1:1081) 代理，SOCKS 端口探测确认连通。**Flutter 侧**：新增 `lib/vpn/` 模块——`vpn_service.dart` 跨平台状态机（idle/authenticating/connecting/connected/disconnecting，凭据持久化一键重连）、`vpn_windows_core.dart` Windows 内核管理、`vpn_page.dart` 液态玻璃风格页面（大圆钮连接按钮 + 呼吸动画 + 错误提示 + 账号配置 + 接入信息卡）；入口注册于 `app_data.dart` 服务分类「校园VPN」（requiresLogin，预填统一身份账号）。原生通道 `com.smartcampus.smartcampus/vpn`（connect/disconnect + onVpnEvent 事件）。Manifest 新增 `FOREGROUND_SERVICE_SYSTEM_EXEMPTED` 权限与 VpnService 声明；Proguard keep go/mobile 包。
...
- **「课程表」更名为「我的课表」**：应用入口名称、页面 AppBar 标题、游客模式拦截提示同步更名；常用功能配置（`home_quick_apps` 按名称持久化）新增历史名称迁移映射（`QuickAppsStore.renamedApps`），旧配置中的「课程表」自动映射为「我的课表」，避免条目被静默过滤。
- **VPN 页面精简与账号输入修复**：移除「分流模式」「关于实现」说明项及底部「仅校内资源经 VPN 隧道转发…」提示文案，页面聚焦连接按钮 + 账号 + 接入信息；账号/密码输入框由 `labelText` 浮动标签改为 `hintText` 占位符（浮动标签在毛玻璃卡片圆角裁剪下被遮挡导致用户名不可见）。
...
- **VPN 登录失败提示不友好（验证码错误只显示笼统失败）**：新增 `_mapVpnError()` 错误映射——Go 侧原始错误串含深信服 ErrorCode 20023（`CAPTCHA incorrect`）时，页面错误卡显示「验证码错误或已过期，请重试」；其余错误剔除 `Login failed:` 前缀与 XML 噪音后展示关键信息。同时修复兜底消息覆盖竞态：登录失败时原生错误事件先于结果回调到达，`connect()` 的通用兜底文案改为仅在无具体信息时写入（`??=`），不再冲掉「验证码错误」等精确提示。
- **VPN 连接成功后无法断开（点断开反而重新弹验证码）**：`_toggle()` 仅凭 `_busy`（只覆盖认证/建隧道中间态）判断走连接还是断开——已连接态下 `_busy=false`，点击「断开」误入连接分支重新 login 触发图形验证码。修复：改为按 `VpnService.phase.value` 判断，connected 态直接走 `disconnect()`。
- **页面返回时偶发 "setState() or markNeedsBuild() called when widget tree was locked" 异常**：`LiquidBackground` 的 `dispose()` 在 `finalizeTree`（widget 树锁定）期间同步递减 `pageBgCount`，全局垫底层的 `ValueListenableBuilder<int>` 在锁定态被通知重建而抛异常（与此前 initState 延迟递增同理）。修复：递减同样延迟到 `addPostFrameCallback` 本帧结束后执行。
...
- **Windows 安装版打开后无窗口但进程在运行**：根因是 `windows/runner/flutter_window.cpp` 依赖 `SetNextFrameCallback` 首帧回调才调用 `Show()` 显示主窗口——当 `FlutterViewController` 构造期间引擎已抢先完成首帧、且 `ForceRedraw()` 在部分时序下为 no-op 时，"下一帧"回调永远不触发，主窗口保持隐藏（Flutter 视图虽正常渲染 UI 但被隐藏的父窗口遮挡，进程消息循环与动画继续运行）。修复：`OnCreate()` 末尾兜底直接 `this->Show()`，窗口创建后立即显示、UI 随后渲染，不再依赖首帧回调。
...
- **周课表网格卡片信息完整显示（内部自适应）**：卡片尺寸不变，课程名改 `Flexible` 约束高度（占剩余空间、高缩放/低单元格高度下不再溢出），字号整体调小——课程名 11→10、教师/教室 8→7.5、类型标签 7→6.5；行数放宽：课程名最多 5 行、教室位置最多 3 行（窄列换行完整显示，不再省略）；默认配置下课程名/教师/教室均完整展示。设置页「实时预览」卡片同步同款字号与约束保持一致。
- **周课表每两小节合并显示（个人课表 + 全校课表）**：`CourseScheduleGrid` 新增 `mergeSections`（默认开启）——1、2 节 / 3、4 节… 每两小节课合并为一个显示单元，网格行数减半更紧凑，左侧节次标签同步改为「1-2节」式，课程卡片按所属大节单元撑满；个人课表与全校课表详情统一生效。
- **全校课表班级卡片信息精简**：卡片副标题由「学院 · 专业 · 年级」改为仅显示专业（`zymc`），班级卡片更简洁。
- **课表表头日期优化**：格式 `M/d` → `M.dd`（点分隔 + 月/日补零，如 8/14 → 8.14，观感更整洁）；字号 9 → 10（跟随文字缩放）并加字重；非今天日期颜色由硬编码 `Colors.black87` 改为 `textSecondary(context)`（**深色模式下黑字不可见问题**，个人课表与班级课表统一生效）。
- **课程详情弹窗信息行图标升级**：教师/教室/时间/周次四行图标由灰色小图标改为**彩色圆角底容器**（32×32 圆角 9 + 10% 透明底色 + 语义配色，与设置页 `IosListTile` 风格统一）——教师蓝、教室绿、时间橙、周次 teal、备注 indigo；行距同步加宽。
- **设置页「外观」条目简介更新**：副标题由「切换浅色/深色模式」改为「明暗模式 · 主题色 · 自定义背景」，与外观页实际能力（主题模式/主题色/自定义背景）一致。
- **课表节次列宽度统一 30px**：原合并模式 35px / 非合并模式 44px 两分支统一为 30px（个人课表与班级课表共用网格同时生效）；节次文字加 `maxLines: 1` + 截断兜底，防 30px 窄列 + 大字号（textScale 上限 1.5）下「1-2节」折行破版。

### 🐛 修复

- **深色模式设置页黑字（外观/常用功能/退出登录/交流群/隐私协议/检查更新/更新日志/作者不可见）**：根因是 `cupertinoOverrideTheme` 未显式传 `brightness`——`CupertinoTheme.brightnessOf` 回退 `MediaQuery.platformBrightnessOf`（系统亮度），App 强制深色但系统浅色时 `GlassListTile` 内部 `CupertinoColors.label` 动态色解析为黑色。**修复：`main.dart` 的 `cupertinoOverrideTheme` 补 `brightness: brightness`（全局根治所有 Cupertino 动态色解析）**；同时 `IosListTile`（设置页全部条目）、常用功能添加弹窗、常用功能管理页三处 `GlassListTile` 的 title 显式 `color: textPrimary(context)` 双重保障。

## [1.2.0] - 2026-08-09

### ✨ 新增

- **全校课表查询显示未排课班级 + 排课状态筛选**：`bjcx.do` 请求补 `querySetting=[]`（网页端固定参数，缺失时服务端按默认条件过滤、未排课班级 SFYPK=0 被吞）；班级列表新增「全部 / 已排课 / 未排课」玻璃筛选 chips（与学院筛选同款），配合原有卡片徽章可精确查看未排课班级。
