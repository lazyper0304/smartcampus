...
/// 宜宾学院邮件系统内置 WebView 页面
///
/// 邮件系统（mailmid.yibinu.edu.cn）使用 phpCAS 接入学校统一认证：
/// 从 SharedHttpClient 提取已登录时的 CASTGC（TGC）注入 WebView，
/// 访问 oauthLogin → authserver CAS 自动放行 → 免密进入邮箱。
///
/// 兜底：CASTGC 在服务端过期时 WebView 会停在 CAS 登录页，
/// 用户可直接在页面内输入账号密码登录，由服务端 Set-Cookie 建立真实会话。
...
class _MailPageState extends State<MailPage> {
  ...
  String _currentTitle = '邮件系统';
  bool _onCasLogin = false;

  @override
  void initState() {
    super.initState();
    _service = MailService(client: widget.client);
    _injectCookies();
  }

  /// 注入统一认证 cookie 到 WebView，完成后加载 oauthLogin
  ...
  bool _isCasLoginUrl(String url) {
    return url.contains('authserver.yibinu.edu.cn') && url.contains('login');
  }
  ...
  Widget _buildWebView() {
    return Column(
      children: [
        ...
        Expanded(
          child: InAppWebView(
            initialUrlRequest: URLRequest(url: WebUri(MailService.baseUrl)),
            ...
            onJsAlert: (ctrl, jsAlertRequest) async {
              debugPrint(
                  'Mail JS alert suppressed: ${jsAlertRequest.message}');
            ...
            onReceivedError: (ctrl, req, err) {
              debugPrint(
                  'Mail WebView error: ${err.type} - ${err.description}');
            },
          ),
        ),
  ...
  Widget _buildBottomToolbar() {
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        border: Border(
          top: BorderSide(
              color: accentColorNotifier.value.withValues(alpha: 0.15)),
        ),
      ),
        ...
        children: [
          ...
          _ToolbarButton(
            ...
            onTap: () => _controller?.loadUrl(
              urlRequest: URLRequest(url: WebUri(MailService.homeUrl)),
            ),
          ),
          _ToolbarButton(
  ...
  Future<void> _clearCache() async {
    ...
    // 清缓存后重注入统一认证 cookie，避免把 SSO 会话也一起清掉
    final count =
        await _service.injectCasCookiesToWebView(CookieManager.instance());
    _controller?.reload();
    if (mounted) {
            ...
            children: [
              ...
              Text(label,
                  style: TextStyle(
                      fontSize: 10,
                      color:
                          enabled ? accentColorNotifier.value : Colors.grey)),
            ],
          ),
        ),
      ),
